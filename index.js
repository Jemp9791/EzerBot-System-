/**
 * TODO_QUESO - Bot Telegram + Config (saludo vendedor) + Catálogo carrusel (1 solo mensaje editable) + Compartir (sin email) + Ayuda
 *
 * ENV (Render):
 * - TELEGRAM_TOKEN   = ...
 * - PUBLIC_URL       = https://xxxxx.onrender.com   (sin barra final)
 ...
 *   list: [],
 *   index: number,
 *   messageId: number|null,   // mensaje del carrusel (SIEMPRE 1)
 *   messageIsPhoto: boolean,  // si ese messageId actualmente es una foto
 *   shareMode: boolean
 * }
 */
...
function mainMenuKeyboardReply() {
  // Menú simple como pediste: sin Carrito, sin Finalizar, con Ayuda
  return {
    keyboard: [
      [{ text: "🛍️ Catálogo" }],
...
function productCaption(item, pos, total) {
  ...
  return (
    ...
    `📌 <i>${pos} de ${total}</i>` +
    `${desc}\n\n` +
    `✨ Tocá <b>Quiero este</b> y te pregunto la cantidad 😊`
  );
}

function productNavKeyboardClean() {
  // Limpio: navegar + quiero + compartir + volver
  return {
    inline_keyboard: [
      [
...
function safeCodeForPayload(code) {
  // payload seguro: solo letras/números/_-
  return String(code || "")
    .trim()
    .replace(/\s+/g, "_")
...
function shareTextForProduct(item) {
  ...
  const code = safeCodeForPayload(item.codigo || "");
  const payload = `P_${code}`; // completo, sin cortar
  ...
  return (
    ...
    `Abrilo y pedilo por el bot 👉 ${link}`
  );
}
...
async function handleStart(chat_id, payload = "") {
  ...
  const instagram = cfg.INSTAGRAM || cfg.IG || "";
  ...
  const info = [
    `👋 <b>¡Hola! Bienvenido/a a ${escapeHtml(negocio)}</b> 🧀`,
    ...
    `🏷️ En <b>Sellos</b> ves tus beneficios.`,
    `📣 Y podés <b>Compartir el bot</b> para invitar a alguien 😉`,
  ]
    .filter(Boolean)
    .join("\n");

  // payload producto compartido
...
async function handleHelp(chat_id) {
  ...
  const txt = [
    `🆘 <b>Ayuda</b>`,
    ``,
    `Estoy para ayudarte a comprar fácil y rápido en <b>${escapeHtml(negocio)}</b> 😊`,
    ``,
    `✅ <b>Cómo comprar</b>`,
    `1) Tocá <b>Catálogo</b>`,
    `2) Elegí una categoría y navegá con <b>Anterior / Siguiente</b>`,
    `3) Tocá <b>Quiero este</b> y te digo cómo pasar la cantidad`,
    ``,
    `📍 <b>Local</b>`,
    direccion ? `• Dirección: ${escapeHtml(direccion)}` : "",
    horarios ? `• Horario: ${escapeHtml(horarios)}` : "",
    telefono ? `• WhatsApp/Teléfono: ${escapeHtml(telefono)}` : "",
    instagram ? `• Instagram: ${escapeHtml(instagram)}` : "",
    ``,
    `💛 Tip: si querés, compartime un producto con el botón <b>Compartir</b> así te lo guardás o se lo mandás a alguien.`,
  ]
    .filter(Boolean)
    .join("\n");
...
async function handleSellos(chat_id) {
  ...
  const txt =
    ...
    `✨ Cada compra suma sellos y te acerca a beneficios.\n` +
    `📣 Tip: compartí el bot o una promo para invitar a alguien 😉`;
...
async function showProductCarousel(chat_id, list, index) {
  ...
  const img = item.imagen && item.imagen.startsWith("http") ? item.imagen : "";
...
/**
 * ✅ Update robusto:
 * - Si el mensaje era texto y ahora hay imagen: Telegram NO deja convertir -> se crea uno nuevo con foto y se actualiza state.messageId
 * - Si era foto y ahora no hay imagen: se intenta editar caption; si falla, manda texto nuevo.
 */
async function updateCarousel(chat_id, st) {
  ...
  if (!st.messageId) {
    ...
    st.messageIsPhoto = created.isPhoto;
  ...
  // Caso con imagen
  if (img) {
    // Si el mensaje actual es texto, NO se puede editar a media -> crear nuevo con foto
    if (!st.messageIsPhoto) {
      const created = await showProductCarousel(chat_id, list, index);
      st.messageId = created.messageId;
      st.messageIsPhoto = created.isPhoto;
      userState.set(chat_id, st);
    ...
    // Si ya era foto, editar media normalmente
    const r = await editMessageMedia(chat_id, st.messageId, img, caption, { reply_markup: kb });
    if (!r?.ok) {
  ...
  // Caso sin imagen
  if (st.messageIsPhoto) {
    // editar caption en foto
    const r = await editMessageCaption(chat_id, st.messageId, caption, { reply_markup: kb });
    if (!r?.ok) {
      // fallback: crear texto nuevo
      const created = await showProductCarousel(chat_id, list, index);
      st.messageId = created.messageId;
      st.messageIsPhoto = created.isPhoto;
      userState.set(chat_id, st);
    ...
    // era texto -> edit text
    const r = await editMessageText(chat_id, st.messageId, caption, { reply_markup: kb });
    if (!r?.ok) {
      const created = await showProductCarousel(chat_id, list, index);
      st.messageId = created.messageId;
      st.messageIsPhoto = created.isPhoto;
...
async function handleCategory(chat_id, category) {
  ...
  // ✅ NO mandar un chat nuevo por categoría si ya hay carrusel: editamos el mismo mensaje
  ...
  const st = {
    ...
    list,
    index: 0,
    messageId: stPrev?.messageId || null,
    messageIsPhoto: stPrev?.messageIsPhoto || false,
    shareMode: false,
  };
...
// ------------------ Compartir producto (submenú limpio, sin email) ------------------
...
async function showSharedProduct(chat_id, codeRaw) {
  const { items } = await loadCatalog();
  const code = String(codeRaw || "").trim().toLowerCase();
  const item = items.find((x) => String(x.codigo || "").trim().toLowerCase() === code);

  if (!item) {
    return sendMessage(chat_id, "🧀 Te compartieron un producto, pero no lo encontré en el catálogo. Entrá a Catálogo para verlo.", {
  ...
  const caption =
    ...
    `\n\n🟢 Si lo querés pedir, tocá <b>Catálogo</b> y elegilo (o escribí <b>QUIERO</b>).`;

  // guardamos estado para que el carrusel quede listo y editable
  const st = {
    mode: "CATALOG",
    categoryLabel: item.categoria || "Todas",
    list: [item],
    index: 0,
    messageId: null,
  ...
  if (item.imagen && item.imagen.startsWith("http")) {
    const m = await sendPhoto(chat_id, item.imagen, caption, {
      parse_mode: "HTML",
      reply_markup: { inline_keyboard: [[{ text: "🛍️ Ver catálogo", callback_data: "CAT_MENU" }]] },
    });
    st.messageId = m?.result?.message_id || null;
    st.messageIsPhoto = true;
    userState.set(chat_id, st);
    return;
  }

  const m = await sendMessage(chat_id, caption, {
    parse_mode: "HTML",
    reply_markup: { inline_keyboard: [[{ text: "🛍️ Ver catálogo", callback_data: "CAT_MENU" }]] },
  });
  st.messageId = m?.result?.message_id || null;
...
async function handleCallback(cb) {
  ...
  if (data === "P:BUY") {
    return sendMessage(chat_id, "🟢 Perfecto 😊\nDecime la cantidad así:\n• <b>200g</b> (gramos)\n• <b>1</b> o <b>2</b> (unidades)\n\nDespués lo hacemos más automático, pero por ahora así queda simple y claro.", {
      parse_mode: "HTML",
      reply_markup: mainMenuKeyboardReply(),
    });
  }
...
async function handleTextMessage(chat_id, text) {
  ...
  if (t.toUpperCase() === "QUIERO") {
    return sendMessage(chat_id, "✅ Dale 😊\nAhora decime la cantidad:\n• <b>200g</b> (gramos)\n• <b>1</b> o <b>2</b> (unidades)\n\n(Ya lo dejamos listo para automatizar después.)", {
      parse_mode: "HTML",
      reply_markup: mainMenuKeyboardReply(),
    });
  }

  return sendMessage(chat_id, "👋 Estoy acá 😊\nTocá <b>Catálogo</b> para ver productos con foto, o <b>Ayuda</b> si querés que te guíe.", {
    parse_mode: "HTML",
    reply_markup: mainMenuKeyboardReply(),
  });
}
...
async function boot() {
  ...
  try {
    ...
    console.log("✅ CATALOGO cargado. Items:", cat.items.length, "Categorías:", cat.categories.length);
  } catch (e) {
